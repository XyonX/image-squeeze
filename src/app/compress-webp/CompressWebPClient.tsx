...
const faqItems = [
	{
		question: "How does WebP compression work?",
		answer: "WebP uses both lossy and lossless compression techniques. Our tool re-encodes your WebP images at your chosen quality level using the browser's built-in WebP encoder, which can significantly reduce file size.",
	},
	{
		question: "Does WebP compression preserve transparency?",
		answer: "Yes! WebP supports alpha channel transparency. When you compress a WebP image with transparency, it remains intact.",
	},
	{
		question: "Is my data safe?",
		answer: "Absolutely. All compression happens in your browser using the Canvas API. Your files never leave your device — we can't see them.",
	},
	{
		question: "What quality setting should I use?",
		answer: "For most web images, 70-80% offers the best balance between quality and file size. For photography, try 80-85%. For icons and graphics, 60-70% works well.",
	},
];

function compressWebP(file: File, quality: number): Promise<Blob> {
...
export function CompressWebPClient() {
	...
	const handleCompress = useCallback(async () => {
		...
		for (let i = 0; i < files.length; i++) {
			try {
				...
				const blob = await compressWebP(files[i].file, quality);
	...
	const downloadSingle = (result: CompressedResult) => {
		saveAs(result.compressedBlob, result.originalName.replace(/\.[^.]+$/, "") + "-compressed.webp");
	};

	const downloadAll = async () => {
		if (results.length <= 1) { if (results[0]) downloadSingle(results[0]); return; }
		const zip = new JSZip();
		results.forEach((r) => zip.file(r.originalName.replace(/\.[^.]+$/, "") + "-compressed.webp", r.compressedBlob));
		saveAs(await zip.generateAsync({ type: "blob" }), "compressed-webp-images.zip");
	};
		...
		<div>
			<div className="text-center mb-8">
				<h1 className="text-3xl sm:text-4xl font-extrabold tracking-tight text-slate-900">
					Compress <span className="text-primary">WebP</span> Online
				</h1>
				<p className="text-slate-500 mt-3 max-w-xl mx-auto">Reduce WebP file size while preserving quality and transparency. Free, private, no signup.</p>
				<div className="mt-4"><TrustBadges /></div>
			</div>

			<UploadZone accept=".webp,image/webp" files={files} onFilesChange={(f) => { setFiles(f); setResults([]); setError(null); }} />
				...
				<div className="mt-8 p-6 bg-white border border-slate-200 rounded-2xl space-y-6">
					<div>
						<div className="flex items-center justify-between mb-2">
							...
							<span className="text-sm text-slate-500">{Math.round(quality * 100)}% — {qualityLabel}</span>
						</div>
