						...
						<div>
							...
							<div className="flex gap-2 mt-3">
								{presetPercentages.map((p) => (
									<button key={p} onClick={() => setPercentage(p)} className={`preset-button px-3 sm:px-4 py-2 rounded-lg text-xs sm:text-sm font-medium text-center border transition-all ${percentage === p ? "border-primary bg-primary/10 text-primary" : "border-slate-200"}`} title={`${p}%`}>{p}%</button>
								))}
							</div>
						</div>
					)}
							...
							<div>
								...
								<div className="flex flex-wrap gap-2">
									{presetDimensions.map((d) => (
										<button key={d.label} onClick={() => { setWidth(d.w); setHeight(d.h); setLockAspect(false); }} className="preset-button px-3 sm:px-4 py-2 rounded-lg text-xs font-medium text-center border border-slate-200 hover:border-primary/30 transition-all text-slate-700" title={d.label}>{d.label}</button>
									))}
								</div>
							</div>
						</div>
