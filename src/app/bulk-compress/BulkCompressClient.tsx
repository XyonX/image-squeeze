...
const faqItems = [
	{
		...
		answer: "You can upload up to 50 images at once. Each file is compressed individually using Web Workers for maximum speed. Download them all as a single ZIP file.",
	},
	{
		question: "What formats are supported?",
		answer: "Bulk compress supports JPG/JPEG, PNG, and WebP images. Each format is compressed with optimal settings automatically.",
	},
	{
		question: "How does batch compression work?",
		answer: "Each image is processed in parallel using Web Workers. The tool auto-detects the format and applies the appropriate compression. Results include individual downloads and a bulk ZIP option.",
	},
	{
		question: "Is my data safe?",
		answer: "Yes. All compression happens in your browser. Your files never leave your device — nothing is uploaded to any server.",
	},
];

export function BulkCompressClient() {
	...
	const handleCompress = useCallback(async () => {
		...
		for (let i = 0; i < files.length; i++) {
			try {
				...
				const saved = ((file.size - compressedFile.size) / file.size) * 100;
				compressed.push({
					id: files[i].id,
				...
				setError(`Failed to compress ${files[i].file.name}. Skipping...`);
			}
		}
	...
	const downloadSingle = (result: CompressedResult) => {
		...
		saveAs(result.compressedBlob, result.originalName.replace(/\.[^.]+$/, "") + `-compressed.${ext}`);
	};
	...
	const downloadAll = async () => {
		...
		results.forEach((r) => {
			...
			zip.file(r.originalName.replace(/\.[^.]+$/, "") + `-compressed.${ext}`, r.compressedBlob);
		});
		saveAs(await zip.generateAsync({ type: "blob" }), "bulk-compressed-images.zip");
	};
		...
		<div>
			<div className="text-center mb-8">
				<h1 className="text-3xl sm:text-4xl font-extrabold tracking-tight text-slate-900">
					<span className="text-primary">Bulk</span> Image Compressor
				</h1>
				<p className="text-slate-500 mt-3 max-w-xl mx-auto">Compress up to 50 images at once. Supports JPG, PNG, and WebP. Download all as ZIP.</p>
