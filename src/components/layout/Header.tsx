...
function LogoIcon() {
	return (
		<svg className="w-7 h-7" viewBox="0 0 200 200" xmlns="http://www.w3.org/2000/svg">
			<defs>
		...
		<header className="sticky top-0 z-50 bg-white/80 backdrop-blur-md border-b border-slate-200">
			<div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
				<div className="flex items-center justify-between h-14">
					{/* Logo */}
					<Link href="/" className="flex items-center gap-2 font-bold text-slate-900 hover:text-slate-700 transition-colors">
						<LogoIcon />
						<span className="hidden sm:inline text-lg">GetImgTools</span>
					</Link>
			...
			{mobileOpen && (
				<div className="md:hidden border-t border-slate-200 bg-white">
					<div className="px-4 py-3 space-y-1 max-h-[70vh] overflow-y-auto">
						<p className="text-xs font-bold text-slate-400 uppercase tracking-widest px-2 py-2">Tools</p>
						{tools.map((tool) => (
							<Link
								key={tool.id}
								href={tool.route}
						...
						<div className="border-t border-slate-200 pt-2 mt-2 space-y-1">
							<Link href="/blog" className="block px-3 py-2 text-sm font-medium text-slate-600 hover:bg-slate-50 rounded-lg" onClick={() => setMobileOpen(false)}>Blog</Link>
							<Link href="/about" className="block px-3 py-2 text-sm font-medium text-slate-600 hover:bg-slate-50 rounded-lg" onClick={() => setMobileOpen(false)}>About</Link>
							<Link href="/privacy" className="block px-3 py-2 text-sm font-medium text-slate-600 hover:bg-slate-50 rounded-lg" onClick={() => setMobileOpen(false)}>Privacy</Link>
						</div>
					</div>
				</div>
			)}
